...
import { factoryImages } from "@/lib/site";
  ...
  return (
    <>
      <PageHero
        ...
        image="/images/site/factory-production-floor.webp"
      />
      <section className="bg-white py-16 sm:py-20">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-8">
          <div className="relative min-h-[480px] overflow-hidden rounded-3xl bg-linen shadow-soft">
            <Image src="/images/site/factory-sewing-line.webp" alt="Pajama factory sewing line" fill className="object-cover" sizes="(min-width:1024px) 50vw, 100vw" />
          </div>
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-clay">Production management</p>
            <h2 className="mt-3 text-3xl font-bold tracking-tight text-cocoa sm:text-4xl">Factory coordination from fabric arrival to packed cartons.</h2>
            <p className="mt-5 text-base leading-8 text-ink/75">Pajama production requires consistent fabric performance, clean sewing, accurate measurements and careful packaging. We manage each stage with approved samples, production specifications, inline checking and final inspection.</p>
            <div className="mt-8 grid gap-4">
              {qc.map((item) => (
                <div key={item.title} className="flex gap-4 rounded-2xl bg-linen p-5">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {factoryImages.map((image) => (
              <div key={image.src} className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-white shadow-sm">
